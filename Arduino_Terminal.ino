...
//                                                                   //
const uint16_t serialport = 115200; //Set Serial Number              //
//                                                                   //
boolean Serialon = true; // Sets Serial on or off                    //
//                                                                   //
/////////////////////////////{Information}/////////////////////////////
//                                                                   //
// @author HKS-HNS                                                   //
...
void loop() {
  ...
  if (Screen == 1) {
    ...
    if (Button  == 0 && digitalRead(WIO_5S_RIGHT) == LOW) { // Button Switch function
      Serial.println("Button Back was Selected on the Menu Screen");
...
void screen(int num) { // to select the screen
  ...
  if (num == 1) { // Menu Screen
    Serial.println("The Menu screen was loaden.");
    ...
    Serial.println("The Start screen was loaden.");
    tft.fillScreen(TFT_GREEN);
    ...
    Serial.println("The Login screen was loaden.");
    tft.setTextSize(3);
    Viewnm = 0;
    Screen = num;
    Dysplay = "";
    ...
    Serial.println("The Server screen was loaden.");
    tft.setTextSize(3);
    Screen = num;
    Button = 0;
    if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("The No wifi Screen was loaden.");
    tft.setTextSize(3);

    Screen = num;
    wifi = false;
    btn0();
  } else if (num == 5) { // Ram Screen
    Serial.println("The Ram Screen was loaden.");
    Usedmem = "MB:";
    tft.setTextSize(3);
    Screen = num;
    wifi = false;
    ...
    Serial.println("The CPU Screen was loaden.");
...
void button(int button1) { // Button control
  if (Screen == 1) {
    if (button1 == 0) {
      Serial.println("Button Back was pressed on the Menu Screen");
      screen(0);
    } else if (button1 == 1) {
      Serial.println("Button Login was pressed on the Menu Screen");
      screen(2);
    }
  } else if (Screen == 2) {
    if (button1 == 1) {
      Serial.println("Button REM was pressed on the Login Screen");
      ...
      Serial.println("Button Back was pressed on the Login Screen");
      screen(1);
    } else if (button1 == 3) {
      if (Dysplay == Password) {
        Serial.println("Button Login was pressed on the Login Screen and the password was correct");
        screen(3);
      } else {
        Serial.println("Button Login was pressed on the Login Screen and the password was incorrect");
        tft.fillRect(0, 0, 320, 50, TFT_DARKGREEN);
        Dysplay = "Error";
        tft.drawString(Dysplay, 5, 17);
      }
    ...
    if (button1 == 0) {
      Serial.println("Button Ram was pressed on the Server Screen ");
      screen(5);
    } else if (button1 == 1) {
      Serial.println("Button CPU was pressed on the Server Screen ");
      screen(6);
    } else if (button1 == 2) {
      Serial.println("Button Back was pressed on the Server Screen ");
      screen(2);
    }
  }
}
